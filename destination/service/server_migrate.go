...
func (s *Server) Migrate(ctx context.Context, in *pb.MigrateRequest) (*pb.MigrateResponse, error) {
	...
	case *pb.MigrationDetails_Drop:
		return handleDropOperation(ctx, conn, schema, table, op.Drop), nil
	case *pb.MigrationDetails_Copy:
		return handleCopyOperation(ctx, conn, schema, table, op.Copy), nil
	case *pb.MigrationDetails_Rename:
		return handleRenameOperation(ctx, conn, schema, table, op.Rename), nil
	case *pb.MigrationDetails_Add:
		return handleAddOperation(ctx, conn, schema, table, op.Add), nil
	case *pb.MigrationDetails_UpdateColumnValue:
		return handleUpdateColumnValue(ctx, conn, schema, table, op.UpdateColumnValue), nil
	case *pb.MigrationDetails_TableSyncModeMigration:
		return handleTableSyncModeMigration(ctx, conn, schema, table, op.TableSyncModeMigration), nil
	default:
...
func handleDropOperation(
	...
	case *pb.DropOperation_DropTable:
		...
		if err != nil {
			return FailedMigrateResponse(schema, table, err)
		}
		...
		return SuccessfulMigrateResponse()
	...
	case *pb.DropOperation_DropColumnInHistoryMode:
		...
		if col == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("drop_column_in_history_mode.column is required"))
		}
...
func handleCopyOperation(
	...
	case *pb.CopyOperation_CopyTable:
		...
		if fromTable == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("copy_table.from_table is required"))
		}
		if toTable == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("copy_table.to_table is required"))
		}
	...
	case *pb.CopyOperation_CopyColumn:
		...
		if fromCol == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("copy_column.from_column is required"))
		}
		if toCol == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("copy_column.to_column is required"))
		}
	...
	case *pb.CopyOperation_CopyTableToHistoryMode:
		...
		if fromTable == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("copy_table_to_history_mode.from_table is required"))
		}
		if toTable == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("copy_table_to_history_mode.to_table is required"))
		}
...
func handleRenameOperation(
	...
	case *pb.RenameOperation_RenameTable:
		...
		if fromTable == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("rename_table.from_table is required"))
		}
		if toTable == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("rename_table.to_table is required"))
		}
	...
	case *pb.RenameOperation_RenameColumn:
		...
		if fromCol == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("rename_column.from_column is required"))
		}
		if toCol == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("rename_column.to_column is required"))
		}
...
func handleAddOperation(
	...
	case *pb.AddOperation_AddColumnWithDefaultValue:
		...
		if col == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("add_column_with_default_value.column is required"))
		}
	...
	case *pb.AddOperation_AddColumnInHistoryMode:
		...
		if col == "" {
			return FailedMigrateResponse(schema, table, fmt.Errorf("add_column_in_history_mode.column is required"))
		}
		chType, err := migrateColumnType(colType)
		if err != nil {
			return FailedMigrateResponse(schema, table, err)
...
func handleUpdateColumnValue(
	...
	if col == "" {
		return FailedMigrateResponse(schema, table, fmt.Errorf("update_column_value.column is required"))
	}
...
func handleTableSyncModeMigration(
	...
	case pb.TableSyncModeMigrationType_SOFT_DELETE_TO_HISTORY:
		// Pass soft_deleted_column through as-is. The DB layer uses it only
		// when supplied and present in the source table; otherwise every source
	...
	case pb.TableSyncModeMigrationType_SOFT_DELETE_TO_LIVE,
		...
		return UnsupportedMigrateResponse()
