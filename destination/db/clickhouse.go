...
// using the query generated by sql.GetInactiveReplicaQuery, and retrying it until all the replicas are active,
...
func (conn *ClickHouseConnection) WaitAllNodesAvailable(
	...
	query, err := sql.GetInactiveReplicaQuery(schemaName, tableName)
	if err != nil {
		return err
	}

	err = retry.OnFalseWithFixedDelay(func() (bool, error) {
		rows, err := conn.ExecQuery(ctx, query, getInactiveReplicas, false)
		if err != nil {
			return false, err
		}
		defer rows.Close()
		...
		var hasInactiveReplica bool
		if err = rows.Scan(&hasInactiveReplica); err != nil {
			return false, err
		}
		return hasInactiveReplica, nil
...
const (
	...
	getInactiveReplicas        connectionOpType = "GetInactiveReplicas"
)
